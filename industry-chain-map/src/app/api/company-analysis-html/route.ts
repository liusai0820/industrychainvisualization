...
// 添加模型环境变量配置
const HTML_MODEL = process.env.COMPANY_HTML_MODEL || "anthropic/claude-3.7-sonnet";
const HTML_TEMPERATURE = parseFloat(process.env.COMPANY_HTML_TEMPERATURE || "0.7");
  ...
  try {
    ...
    try {
      ...
      if (OPENROUTER_API_KEY) {
        ...
        // 创建请求LLM生成HTML的提示，使用导入的函数
        const htmlPrompt = generateCompanyAnalysisHTMLPrompt({
          companyName, 
...
async function callOpenRouterForHTML(prompt: string) {
  ...
  const payload = {
    ...
    "temperature": HTML_TEMPERATURE,  // 使用环境变量
    "top_p": 1,
    "frequency_penalty": 0,
    "presence_penalty": 0,
    "stream": false,
